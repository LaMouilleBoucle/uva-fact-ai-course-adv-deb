...
class AdultUCI(Dataset):
    ...
    def process_data(self):
        ...
        for idx, name in enumerate(self.var_names):
            if name not in self.real_var_names:
                _, one_hot_idx, _ = self.one_hot_encode(self.data[name])
                if name is 'income':
                    self.labels = torch.tensor(one_hot_idx)
                    continue
                elif name in self.protected_var_names:
                    if 'protected_temp' in locals():
                        protected_temp = np.concatenate((protected_temp, one_hot_idx), axis=1)
                    else:
                        protected_temp = one_hot_idx
                    continue
                if idx == 0:
                    data_temp = one_hot_idx
                    continue
                else:
                    data_temp = np.concatenate((data_temp, one_hot_idx), axis=1)
            else:
                if idx == 0:
                    data_temp = self.data[name].values[:,None]
                    continue
                elif name is 'fnlwgt':
                    continue
                else:
                    data_temp = np.concatenate((data_temp, self.data[name].values[:,None]), axis=1)
        self.data = torch.tensor(data_temp).float()
        self.protected = torch.tensor(protected_temp).float()
    ...
    def one_hot_encode(self, data):
        ...
        return encoder, np.array(one_hot_idx)[:,None], one_hot
